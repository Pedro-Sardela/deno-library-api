...
> {
  ...
  findById(id: string | Types.ObjectId, options?: QueryOptions) {
    if (!is.objectId(id)) {
      throw throwlhos.err_internalServerError('findById requer um ObjectId válido!', {
        givenId: id,
        typeofGivenObjectId: typeof id,
      }, {
        key: 'repositories.baseRepository.findByIdRequiresValidObjectId',
      })
    }
  ...
  updateById(
    ...
    if (!is.objectId(id)) {
      throw throwlhos.err_internalServerError('updateById requer um ObjectId válido!', {
        givenId: id,
        typeofGivenObjectId: typeof id,
      }, {
        key: 'repositories.baseRepository.updateByIdRequiresValidObjectId',
      })
    }
  ...
  paginate<D = T>(
    ...
    if (!this.model.aggregatePaginate) {
      throw throwlhos.err_internalServerError(
        'Model não tem o método aggregatePaginate. Verifique se o plugin de paginação foi aplicado ao schema.',
        undefined,
        {
          key: 'repositories.baseRepository.modelMissingAggregatePaginate',
        },
      )
    }
